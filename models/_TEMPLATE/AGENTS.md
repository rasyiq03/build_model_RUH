# AGENTS.md — RUH Model Spec: {{MODEL_NAME}}
...
# >>> USER OVERRIDE {{DATE}} (ABSOLUTE — supersedes any conflicting text below):
...
Orchestrator: `generate_{{MODEL_SLUG}}.py` — its REGISTRY lists every component above.
...
One FBX per collection into `models/{{MODEL_SLUG}}/exports/`.
...
- [ ] `generate_{{MODEL_SLUG}}.py` runs from a clean Blender, zero tracebacks, OVERALL `OK`.
